...
for file in config = %w{ workling.yml } do
  FileUtils.cp(File.join(plugin_root, 'config', file), Rails.root.join('config'))
end

# scripts
for file in script = %w{ workling_starling_client workling_client bj_invoker.rb starling_status.rb } do
  FileUtils.cp File.join(plugin_root, 'script', file), Rails.root.join('script')
  FileUtils.chmod 0755, Rails.root.join('script', file)
end
